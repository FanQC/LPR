...
class LPR():
    ...
    def model_seq_rec(self,model_path):
        ...
        # class+1 有一类为背景，n_len=7 中文车牌字符长度都为7
        rnn_size = 256
    ...
    def model_finemapping(self):
        ...
        # Dense→Keras的全连接层，2为输出维度
        output = Activation("relu", name='relu4')(output)
        model = Model([input], [output])
        # Model(input, output) 构造函数输出模型
        return model
    ...
    def recognizeOne(self,src):
        ...
        # 矩阵转置，按照1,0,2顺序排列，（w,h,channel）
        y_pred = self.modelSeqRec.predict(np.array([x_temp]))
        # 使用GRU判断字符
        y_pred = y_pred[:,1:,:]
        # 为什么从第三个开始取，因为总结了规律，通常第二、三个都是一样的中文字
